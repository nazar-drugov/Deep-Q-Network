"""
Plotting utilities for reinforcement learning (DQN and related).

Provides conversion helpers (to_numpy), generic Plotly wrappers (imshow, line,
scatter, bar, hist), and RL-specific visualizations (cliffwalk_imshow,
plot_cartpole_obs_and_dones) used by the deep_q_nets pipeline.
"""
...
# ============================================================================
# Conversion and Layout Helpers
# ============================================================================


def to_numpy(tensor: "t.Tensor | np.ndarray | list | tuple | int | float | bool | str") -> np.ndarray:
    """
    Purpose:
        Convert tensor-like input to a numpy array. Accepts PyTorch tensors,
        numpy arrays, lists, tuples, and scalars.

    Parameters:
     * tensor : input to convert (Tensor, ndarray, list, tuple, or scalar)
...
def imshow(tensor: t.Tensor, renderer=None, **kwargs):
    """
    Purpose:
        Display a 2D or 3D tensor as a Plotly heatmap (imshow). Supports
        facet_labels, border, text overlay, and layout kwargs.

    Parameters:
     * tensor (t.Tensor) : 2D or 3D data to display
     * renderer : Plotly renderer (default: None)
     * **kwargs : passed to px.imshow or layout (update_layout_set)

    Returns:
        Plotly figure, or None if showing inline (return_fig=False).
    """
...
def reorder_list_in_plotly_way(L: list, col_wrap: int) -> list:
    """
    Purpose:
        Reorder a list to match Plotly's facet layout when using column wrap.

    Parameters:
     * L (list) : labels or items to reorder
     * col_wrap (int) : number of columns in the facet layout

    Returns:
        Reordered list matching Plotly's facet order.
    """
    L_new = []
    while len(L) > 0:
...
def line(y: t.Tensor | list, renderer=None, **kwargs):
    """
    Purpose:
        Plot y as a line chart via Plotly. Supports layout kwargs (e.g. yaxis_range)
        and optional secondary y-axis.

    Parameters:
     * y (t.Tensor | list) : 1D or 2D data (series) to plot
     * renderer : Plotly renderer (default: None)
     * **kwargs : passed to px.line or layout (update_layout_set)
...
def scatter(x, y, renderer=None, **kwargs):
    """
    Purpose:
        Scatter plot of x vs y via Plotly. Supports facet_labels and add_line
        (e.g. 'x=y', 'x=c', 'y=c').

    Parameters:
     * x, y : data for x- and y-axes (tensor-like, converted via to_numpy)
     * renderer : Plotly renderer (default: None)
     * **kwargs : passed to px.scatter or layout (update_layout_set)
...
def bar(tensor, renderer=None, **kwargs):
    """
    Purpose:
        Bar chart of tensor values via Plotly.

    Parameters:
     * tensor : 1D data (tensor-like, converted via to_numpy)
     * renderer : Plotly renderer (default: None)
     * **kwargs : passed to px.bar or layout (update_layout_set)
...
def hist(tensor, renderer=None, **kwargs):
    """
    Purpose:
        Histogram of tensor values via Plotly.

    Parameters:
     * tensor : 1D data (tensor-like, converted via to_numpy)
     * renderer : Plotly renderer (default: None)
     * **kwargs : passed to px.histogram or layout (update_layout_set)
...
# ============================================================================
# RL-Specific Plotting Functions
# ============================================================================
...
def cliffwalk_imshow(tensor, pi, renderer=None, filename: str | None = None, **kwargs):
    """
    Purpose:
        Plot a Cliffwalk-style value grid as a heatmap with policy arrows
        overlaid. Each cell shows value and an arrow indicating the greedy action.

    Parameters:
     * tensor : 2D value array (tensor-like, converted via to_numpy)
     * pi : 2D policy array (action indices 0..3 per cell; same shape as tensor)
     * renderer : Plotly renderer (default: None)
     * filename (str | None) : if set, save figure as HTML to this path
     * **kwargs : passed to px.imshow or layout (update_layout_set)

    Returns:
        None (displays figure; optionally writes HTML to filename).
    """
    kwargs_post = {k: v for k, v in kwargs.items() if k in update_layout_set}
    kwargs_pre = {k: v for k, v in kwargs.items() if k not in update_layout_set}
    if "color_continuous_scale" not in kwargs_pre:
...
def plot_cartpole_obs_and_dones(
    ...
    """
    Purpose:
        Plot CartPole observations (cart position, pole angle) over time with
        vertical dashed lines at episode terminations. Supports single or
        vectorized (n_envs) traces.

    Parameters:
     * obs (np.ndarray | t.Tensor) : shape (n_steps, n_envs, n_obs) or (n_steps, n_obs)
     * done (np.ndarray | t.Tensor) : shape (n_steps, n_envs) or (n_steps,);
                                     done flags per step (truthy = termination)
     * title (str) : plot title (default: "CartPole experiences (dotted lines = termination)")
     * filename (str | None) : if set, save figure as HTML to this path
