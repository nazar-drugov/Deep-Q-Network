"""
Vectorized Gymnasium environments that run on GPU (PyTorch tensors).

Provides CartPole (and optionally Pendulum) for use with DQN trajectory
visualization (e.g., generate_and_plot_trajectory in rl_utils). CartPole state
and dynamics use torch tensors on a specified device.

CartPole: Classic cart-pole system (Barto, Sutton, Anderson). Supports
vectorized env_count; step/reset return (state, reward, terminated, truncated, info).
Pendulum: Legacy implementation; may rely on deprecated gym APIs and is not
used by the DQN pipeline.

Reference: https://gist.github.com/ngoodger/6cf50a05c9b3c189be30fab34ab5d85c
"""
...
# ============================================================================
# Pendulum (legacy; uses old gym APIs, not used by DQN)
# ============================================================================


class Pendulum(gym.Env):
    """
    Vectorized Pendulum (inverted pendulum) env. Legacy implementation;
    uses old gym APIs (seeding, rendering) and is not used by the DQN pipeline.
    """
...
# ============================================================================
# CartPole (vectorized, GPU-friendly)
# ============================================================================
# Classic cart-pole system (Barto, Sutton, Anderson).
# Ref: http://incompleteideas.net/sutton/book/code/pole.c
...
class CartPole(gym.Env):
    ...
    def step(self, action):
        ...
        Parameters:
         * action : tensor or array of shape (env_count,) or (env_count, 1);
                    discrete action 0 = left, 1 = right per env.

        Returns:
            (state, reward, terminated, truncated, infos) in gymnasium style.
        """
    ...
    def reset(self, seed=None, options=None):
        """
        Reset environments: resample state for terminated/truncated envs,
        keep others. Returns (state, infos) with infos["final_observation"].

        Parameters:
         * seed : optional RNG seed (unused; compatibility).
         * options : optional dict (unused; compatibility).

        Returns:
            (state, infos) with state shape (env_count, 4).
        """
        done = self.truncated | self.terminated
        new_state = torch.where(
            done.unsqueeze(1),
            (torch.rand(self.env_count, 4, device=self.device) - 0.5) / 10.0,
            self.state,
        )
    ...
    def render(self, env_idx=0):
        """
        Render the env_idx-th environment as an RGB array (H, W, 3).

        Parameters:
         * env_idx (int) : index of environment to render (default: 0).

        Returns:
            np.ndarray of shape (screen_height, screen_width, 3), or None if
            state is None.
        """
    ...
    def close(self):
        """Clean up resources. No-op for CartPole (PIL-based rendering)."""
        pass